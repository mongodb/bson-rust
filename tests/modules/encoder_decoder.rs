...
fn test_encode_decode_floating_point() {
    ...
    let decoded = Document::decode_document(&mut Cursor::new(buf)).unwrap();
...
fn test_decode_utc_date_time_overflows() {
    ...
    let decoded = Document::decode_document(&mut Cursor::new(raw)).unwrap();
...
fn test_decode_invalid_utf8_string_issue64() {
    ...
    assert!(Document::decode_document(&mut Cursor::new(buffer)).is_err());
}
