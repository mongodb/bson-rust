...
/// Wrapper that causes the contained type to always serialize and deserialize
/// as if the format is human-readible.
...
impl<T: Serialize> Serialize for HumanReadable<T> {
    ...
    where
        S: Serializer,
    {
        self.0.serialize(HumanReadableSerializer(serializer))
    }
}
...
impl<'de, T: Deserialize<'de>> Deserialize<'de> for HumanReadable<T> {
    ...
    {
        T::deserialize(HumanReadableDeserializer(deserializer)).map(Self)
    }
}

struct HumanReadableSerializer<S>(S);
...
struct HumanReadableDeserializer<D>(D);

impl<'de, D: Deserializer<'de>> Deserializer<'de> for HumanReadableDeserializer<D> {
    type Error = D::Error;
