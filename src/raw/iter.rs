...
/// A view into a value contained in a [`RawDocument`]. The underlying bytes of the element are not
/// parsed or validated; call [`RawElement::value`] or one of the `TryFrom` implementations to
/// convert the element into a BSON value.
#[derive(Clone)]
