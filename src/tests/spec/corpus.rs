...
use crate::{raw::RawDocumentRef, tests::LOCK, Bson, Document};
...
fn run_test(test: TestFile) {
    ...
    for valid in test.valid {
        ...
        let document_from_raw_document: Document = RawDocumentRef::new(canonical_bson.as_slice())
            .expect(&description)
            .try_into()
            .expect(&description);
        ...
        if let Some(db) = valid.degenerate_bson {
            ...
            let document_from_raw_document: Document = RawDocumentRef::new(db.as_slice())
                .expect(&description)
                .try_into()
                .expect(&description);
            let mut documenttowriter_document_from_raw_document = Vec::new();
